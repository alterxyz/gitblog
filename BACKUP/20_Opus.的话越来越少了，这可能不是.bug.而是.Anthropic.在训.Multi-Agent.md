...
> 关于通信约束的 prompt 目前是 fake 阶段的表现，那么 JSON 文件通信这件事情呢？
> train 阶段，会怎样改变 Agent 间的通信？

Fake 的够用也行，渐进式的也能用。 JSON 文件通信是 AI system （Agent Team） 或者说实现层的工程体现，可能没那么重要。在模型层面的范式变化可能才是他们的核心。

（之前听说 Gemini 的一些功能不开发，不像 Claude 那种 Agent 可能是和他们的训练大方向不符。）

![image](https://github.com/user-attachments/assets/57b82e7a-d9c2-4ec8-b1e4-f97d6f64a614)
